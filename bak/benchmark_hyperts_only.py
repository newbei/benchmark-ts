...
if len(sys.argv) > 1:
    ...
    result_file_path = report_base_path + os.sep + 'benchmark_hyperts' + time.strftime("%Y%m%d_%H%M%S",
                                                                                       time.localtime()) + '.csv'
if os.path.exists(result_file_path):
    trained_data_names = pd.read_csv(result_file_path)['dataset'].values

types = ['univariate-forecast', 'multivariate-forecast', 'classification']
...
def trail(TRAIN_APTH, TEST_PATH, Date_Col_Name, Series_Col_name, forecast_length, format, task, metric, covariables):
    ...
    if "class" in task:
        ...
        # Metrics
        return score_calc(y_test, y_pred, metric), time_cost, run_kwargs
        ...
        if os.path.exists(path):
            ...
            for dir in list:
                ...
                if os.path.exists(train_file_path) and os.path.getsize(train_file_path) > 0:
                    ...
                    covariables = config['covariables'].split(",") if 'covariables' in config else None
                    f.close()
